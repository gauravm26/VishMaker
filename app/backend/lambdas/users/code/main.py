...
# Initialize logger
logger = setup_logger('project_api')

def create_app() -> FastAPI:
    """Create and configure the FastAPI application."""
    
    app = FastAPI(
        title=f"{get_project_name()} Users API",
        description="Users API - Handle projects, requirements, waitlist, and user data management",
        version="1.0.0",
    ...
    async def ping():
        ...
        return {"message": "pong", "service": "project_api"}
    
    @app.get("/")
    async def root():
        """Root endpoint."""
        return {
            "message": f"{get_project_name()} Users API",
            "version": "1.0.0",
            "service": "users_api"
        }
...
if __name__ == "__main__":
    ...
    logger.info("Starting Users API in development mode")
    uvicorn.run(
        "main:app",
        host="0.0.0.0", 
        port=8001,  # Different port from LLM API
        reload=True,
        log_level="info"
    ) 