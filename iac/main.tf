...
resource "aws_acm_certificate" "websocket" {
  domain_name = "ws.vishmaker.com" # Minimal required argument
  lifecycle {
    ignore_changes = all
  }
}
...
resource "aws_route53_record" "websocket_cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.websocket.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "websocket" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.websocket_cert_validation : record.fqdn]
  lifecycle {
    ignore_changes = all
  }
}
...
resource "aws_instance" "websocket_server" {
  ...
  tags = merge(local.common_tags, { Name = "${local.name_prefix}-websocket-server" })
  lifecycle { ignore_changes = all }
}

resource "aws_iam_role" "ec2_ecr_access_role" {
  name = "${local.name_prefix}-ec2-ecr-access-role"
...
resource "aws_cloudfront_distribution" "main" {
  ...
  origin {
    ...
    domain_name = aws_lb.ws_alb.dns_name
  ...
  lifecycle {
    # Tell Terraform to ignore just the origins block for now
    ignore_changes = all
  }
}
...
resource "aws_route53_record" "websocket" {
  name = "ws.vishmaker.com"
  type = "A"
  zone_id = data.aws_route53_zone.main.zone_id
  records = [aws_instance.websocket_server.public_ip]
  ttl = 300
  lifecycle { ignore_changes = all }
}
